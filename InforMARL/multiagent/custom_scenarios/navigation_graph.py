...
class Scenario(BaseScenario):
    def make_world(self, args: argparse.Namespace) -> World:
        ...
        # 与宿主（sheep-herding-rl）对接的两项语义开关
        self.external_goals = bool(getattr(args, "external_goals", False))
        self.use_shepherd_env = bool(getattr(args, "use_shepherd_env", False))
        ####################
        world = World()
        ...
        # 把开关同步到 World，便于其他场景方法直接读
        world.external_goals = self.external_goals
        world.use_shepherd_env = self.use_shepherd_env
    ...
    def random_scenario(self, world):
        ...
        if self.external_goals:
            # 由宿主在 reset 之后调用 apply_external_state 写入；
            # 这里只占位为原点，避免 min_time/observation 在外部状态写入前访问空字段。
    ...
    def apply_external_state(
        ...
        agent_positions: arr,
        goal_positions: arr,
        sheep_position: Optional[arr] = None,
    ) -> None:
        """
        宿主（sheep-herding-rl 主环境）在每个主步开始时调用：
        把每只狗的当前位置写入 ``world.agents[i].state.p_pos``，
        把每只狗的目标点写入 ``world.landmarks[i].state.p_pos``，
        若启用 ``use_shepherd_env`` 则把羊群质心写入 ``world.obstacles[0].state.p_pos``。
        最后刷新缓存距离矩阵和 GNN 邻接表，使后续 ``observation``/``graph_observation`` 一致。

        本方法不重置 RNN 状态，也不修改速度（保留宿主期望的 v=0 初始化）。
        """
        ag = np.asarray(agent_positions, dtype=np.float64).reshape(-1, world.dim_p)
        gp = np.asarray(goal_positions, dtype=np.float64).reshape(-1, world.dim_p)
        n_a = min(len(world.agents), ag.shape[0])
        for i in range(n_a):
            world.agents[i].state.p_pos = ag[i].copy()
            world.agents[i].state.p_vel = np.zeros(world.dim_p)
            world.agents[i].state.c = np.zeros(world.dim_c)
        n_g = min(len(world.landmarks), gp.shape[0])
        for i in range(n_g):
            world.landmarks[i].state.p_pos = gp[i].copy()
            world.landmarks[i].state.p_vel = np.zeros(world.dim_p)
        if sheep_position is not None and len(world.obstacles) > 0:
            sp = np.asarray(sheep_position, dtype=np.float64).reshape(world.dim_p)
            world.obstacles[0].state.p_pos = sp.copy()
