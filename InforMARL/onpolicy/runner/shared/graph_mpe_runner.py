...
def _graph_vec_env_world(envs):
    """Unwrap GraphDummyVecEnv (n_rollout_threads==1) to underlying MultiAgentGraphEnv.world."""
    return envs.envs[0].world
...
class GMPERunner(Runner):
    ...
    def render(self, get_metrics: bool = False):
        ...
        """
        envs = self.envs
        if not self.all_args.save_gifs:
            _human_pause = max(
                float(self.all_args.ifi),
                float(getattr(self.all_args, "human_render_sleep", 0.3)),
            )
        else:
            _human_pause = 0.0
        ...
        use_mpl = (
            not get_metrics
            and not self.all_args.save_gifs
            and bool(getattr(self.all_args, "render_with_matplotlib", False))
        )
        mpl_viewer = None
        if use_mpl:
            from onpolicy.utils.graph_mpe_matplotlib import MatplotlibGraphViewer

            cam = float(getattr(self.all_args, "world_size", 2.0))
            _live_png = os.path.join(str(self.all_args.model_dir), "render_live.png")
            _fs = float(getattr(self.all_args, "mpl_figsize", 6.0))
            mpl_viewer = MatplotlibGraphViewer(
                cam_range=cam,
                autosave_path=_live_png,
                autosave_every=int(getattr(self.all_args, "mpl_autosave_every", 20)),
                figsize=(_fs, _fs),
            )
        ...
        try:
            for episode in range(self.all_args.render_episodes):
                ...
                if not get_metrics:
                    if self.all_args.save_gifs:
                        ...
                        mpl_viewer.update(
                            _graph_vec_env_world(envs),
                            title=f"episode {episode} · after reset",
                        )
                        mpl_viewer.pause(_human_pause)
                    else:
                        envs.render("human")
                        time.sleep(_human_pause)
            ...
            if mpl_viewer is not None:
                try:
                    # 与根目录 visualize.py 一致：默认不阻塞；需要关窗再继续时设环境变量
                    if mpl_viewer._interactive and os.environ.get(
                        "INFORMARL_MPL_BLOCK_END", ""
                    ).lower() in ("1", "true", "yes"):
                        mpl_viewer.block_until_closed()
                finally:
                    mpl_viewer.close()
