...
def get_config():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.12,
        help="When rendering with human (not save_gifs), pause this many seconds "
        "after each frame; actual sleep is max(ifi, human_render_sleep). Smaller "
        "values feel more responsive (matplotlib + Tk can feel slow if too large).",
    )
    parser.add_argument(
        "--render_with_matplotlib",
        action="store_true",
        default=False,
        help="When evaluating with human mode (not save_gifs), draw the scene with "
        "matplotlib instead of pyglet. Useful when pyglet windows are unreliable.",
    )
    parser.add_argument(
        "--force_mpl_headless",
        action="store_true",
        default=False,
        help="With --render_with_matplotlib, force Agg backend (no window; same idea "
        "as visualize.py --force_headless).",
    )
    parser.add_argument(
        "--mpl_autosave_every",
        type=int,
        default=20,
        help="With matplotlib + render_live.png: save every N frames (0=disable). "
        "Larger N reduces disk IO stutter.",
    )
    parser.add_argument(
        "--mpl_figsize",
        type=float,
        default=6.0,
        help="Square matplotlib figure side length in inches (smaller = faster).",
    )
    parser.add_argument(
        "--render_eval",
        action="store_true",
...
def graph_config(args, parser):
    ...
    # ----------------- sheep-herding-rl 宿主对接开关 -----------------
    # 默认 False 与上游训练完全一致；启用后由宿主在每个主步通过
    # ``Scenario.apply_external_state`` 写入 agent/landmark/羊群质心。
    parser.add_argument(
        "--external_goals",
        action="store_true",
        default=False,
        help="Whether landmark goals are written by the host at every "
        "macro step (skip random goal init in scenario.reset).",
    )
    parser.add_argument(
        "--use_shepherd_env",
        action="store_true",
        default=False,
        help="Treat obstacles[0] as the sheep flock centroid; host updates "
        "it every macro step via apply_external_state.",
    )
