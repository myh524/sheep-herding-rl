...
def parse_args():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='在 matplotlib 主视图（含 --save-visual-every 保存的 PNG）上绘制每只机械狗自本回合 reset 起的轨迹折线；'
        '线的一端为初始位置（不单独打点），另一端与当前方块位置一致。',
    )
    parser.add_argument(
        '--force_headless',
        action='store_true',
...
class Visualizer:
    ...
    def _init_visual_snapshots_if_needed(self) -> None:
        ...
        extra = ""
        if getattr(self.args, "save_visual_herder_trails", False):
            extra = "；已启用机械狗轨迹折线 (--save-visual-herder-trails)"
        print(
            f"视觉快照: 每 {int(k)} 步保存 → {self._visual_snapshot_dir}{extra}",
            flush=True,
        )
    ...
    def render_env(self):
        ...
        if getattr(self.args, "save_visual_herder_trails", False) and self._herder_trail_buffer:
            traj = np.stack(self._herder_trail_buffer, axis=0)
            if traj.shape[0] >= 2:
    ...
    def run_episode(self, episode_num: int, total_episodes: int):
        ...
        if getattr(self.args, "save_visual_herder_trails", False):
            self._herder_trail_buffer = [
                self.env.scenario.herder_positions.copy().astype(np.float64)
            ]
        else:
            self._herder_trail_buffer = None
        ...
        for step in range(self.env.episode_length):
            ...
            if self._herder_trail_buffer is not None:
                self._herder_trail_buffer.append(
                    self.env.scenario.herder_positions.copy().astype(np.float64)
                )
